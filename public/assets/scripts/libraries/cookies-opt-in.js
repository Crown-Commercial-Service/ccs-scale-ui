...
var oneyear=31540000;var onemonth=2628000;// Set the default cookies. This JSON Object is saved as the cookie, but we use `initial_cookie_preferences` to maintain structure and various sanity checks
var cookie_preferences={essentials:true,usage:true,marketing:true};/**
     ...
     */function optUserIn(){hideMessage();updateSeenCookie()}function updateSeenCookie(){// check if seen_cookie is set, if not, set it (we're checking this first because we don't want to reset to today every time)
if(!docCookies.hasItem("seen_cookie_message")){// 1 year = 3.154e+7
// 1 month = 2.628e+6
// set the cookie which tells us a user has 'accepted cookies'
// setItem: function (sKey, sValue, vEnd, sPath, sDomain, bSecure)
docCookies.setItem("seen_cookie_message",true,oneyear,"/",".crowncommercial.gov.uk");// the old 'seen' cookie is called `seen-cookie-message` (this should probably be deleted
}// Only set or change the cookies if the user hasn't updated their preferences on the Cookie Settings page
if(!docCookies.hasItem("cookie_preferences_set")){var cookie_preferences_accepted={essentials:true,usage:true,marketing:true};docCookies.setItem("cookie_preferences",JSON.stringify(cookie_preferences_accepted),onemonth,"/",".crowncommercial.gov.uk");// createCookie('cookie_preferences', JSON.stringify(cookie_preferences), 365, '/');
...
deleteDisabledCookies(datarecord.cookie_type)}}});docCookies.setItem("cookie_preferences",JSON.stringify(cookie_preferences),onemonth,"/",".crowncommercial.gov.uk");// createCookie('cookie_preferences', JSON.stringify(cookie_preferences), 365, '/');
...
docCookies.setItem("cookie_preferences_set",true,onemonth,"/",".crowncommercial.gov.uk");// createCookie('cookie_preferences_set', 'true', 365, '/');
...
var cookieMessageInner=document.createElement("div");cookieMessageInner.classList.add("cookie-message__inner","govuk-width-container");cookieMessageInner.classList.add("site-container");cookieMessageInner.innerHTML="<div class=\"cookie-message__intro\"><p>We use cookies to collect information about how you use crowncommercial.gov.uk. We use this information to make the website work as well as possible and improve government services.</p></div>";var optInButton=document.createElement("button");optInButton.classList.add("govuk-!-font-size-18","govuk-!-font-weight-bold","govuk-button","gtm--accept-cookies-in-banner");optInButton.innerHTML="Accept cookies";optInButton.addEventListener("click",optUserIn);var settingsButton=document.createElement("a");settingsButton.setAttribute("href","/cookie-settings");settingsButton.innerHTML="Cookie settings";// optOutButton.classList.add('button');
...
if(!docCookies.hasItem("cookie_preferences")){docCookies.setItem("cookie_preferences",JSON.stringify(cookie_preferences),onemonth,"/",".crowncommercial.gov.uk");// createCookie('cookie_preferences', JSON.stringify(cookie_preferences), 365, '/');
}/**
